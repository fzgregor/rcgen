	...
	pub fn from_params(mut params :CertificateParams) -> Result<Self, RcgenError> {
		let key_pair = if let Some(key_pair) = params.key_pair.take() {
			if !key_pair.is_compatible(params.alg) {
				return Err(RcgenError::CertificateKeyMismatch);
			}
			key_pair
		} else {
...
pub enum RcgenError {
	...
	RingUnspecified,
	/// The given key pair doesn't fit the given certificate
...
impl fmt::Display for RcgenError {
	fn fmt(&self, f :&mut fmt::Formatter) -> fmt::Result {
		...
		match self {
			...
			CertificateKeyMismatch => write!(f, "The given key pair doesn't fit the given \
				certificate")?,
...
impl SignatureAlgorithm {
	...
	fn from_oid(oid :&[u64]) -> Result<&'static SignatureAlgorithm, RcgenError> {
